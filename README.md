...
All wording, including image alt text, lives in `src/content/site.ts`. Nothing
else holds a second copy of it: the page metadata, the Open Graph and Twitter
tags, the JSON-LD, and `/site.webmanifest` all read from that file. The
manifest used to be a static file in `public/` and went stale when the brand
copy changed, so it is now generated by `src/pages/site.webmanifest.ts`.
